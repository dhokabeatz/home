...
export class ContactsController {
  ...
  createAdmin(@Body() createContactDto: CreateContactDto, @Request() req: any) {
    ...
    const contactData = {
      ...createContactDto,
      ipAddress: createContactDto.ipAddress || req.ip || req.connection?.remoteAddress || '127.0.0.1',
      userAgent: createContactDto.userAgent || req.get('User-Agent') || 'Unknown',
    };
    return this.contactsService.create(contactData);
  }
  ...
  @ApiResponse({ status: 200, description: 'Contact submission updated successfully' })
  @ApiResponse({ status: 400, description: 'Bad request' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Contact submission not found' })
